...
$$
\begin{align*}
q^\star(\beta) \propto \exp \left( E_{q(\sigma^2)} \left( \log \left( p(\beta | \mathbf{y}, \sigma^2 ) \right) \right) \right)
\end{align*}
$$
which requires finding the full conditional distribution $p(\beta | \mathbf{y}, \sigma^2 )$. The full conditional distribution is
$$
\begin{align*}
...
& = \exp \left(  E_{q(\sigma^2)}  E_{q(\mu)} \left( -\frac{1}{2} a \left( \beta - a^{-1} b \right)^2 \right) \right)  \\
& = \exp \left(  - \frac{1}{2} E_{q(\sigma^2)} \left( \frac{1}{\sigma^2} \right) \left( \sum_{i=1}^n x_i^2 + \frac{1}{\sigma^2_\beta} \right) E_{q(\mu)} \left( \left( \beta - \frac{\sum_{i=1}^n x_i (y_i - \mu) + \frac{\mu_\beta}{\sigma^2_\beta}}{ \sum_{i=1}^n x_i^2 + \frac{1}{\sigma^2_\beta}} \right)^2 \right) \right) \\
\end{align*}
$$
